...
void BufferIndexedVerts(MeshData& meshdata)
{
   ...
   const int pos_loc = AttribLoc::Pos_loc;
   const int tex_coord_loc = AttribLoc::TexCoord_loc;
   const int normal_loc = AttribLoc::Normal_loc;
...
void SubmeshData::DrawSubmesh()
{
   //glDrawElementsBaseVertex(GL_TRIANGLES, mNumIndices, GL_UNSIGNED_INT, (void*)(sizeof(unsigned int)*mBaseIndex), mBaseVertex);
	glDrawElementsBaseVertex(GL_PATCHES, mNumIndices, GL_UNSIGNED_INT, (void*)(sizeof(unsigned int)*mBaseIndex), mBaseVertex);
}
